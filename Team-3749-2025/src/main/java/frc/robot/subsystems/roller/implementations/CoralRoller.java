...
public class CoralRoller extends Roller {
    ...
    private ShuffleData<Boolean> setInitialStateLog = new ShuffleData<Boolean>(this.getName(), "setInitialState",
            routineStarted);
    ...
    public boolean hasPiece() {
        if (Robot.isSimulation()) {
            ...
            if (!getIsStableState() && super.rollerState == RollerStates.RUN
                    && Math.abs(data.rollerVelocityRadPerSec) < 0.1) {
                hasPiece = true;
            } else {
                hasPiece = false;
            }
