...
import frc.robot.subsystems.rollers.algae.intake.AlgaeIntakeIO.AlgaeIntakeData;;


public class AlgaeIntake extends SubsystemBase {

    private AlgaeIntakeData intakeData = new AlgaeIntakeData();
    private AlgaeIntakeIO intakeIO;
    private double goalVelocity = 0; //consider adding resting voltage
    private RollerStates algaeState = RollerStates.STOP; //btw if this isn't obvious the goal velocity and coral state are 
    ...
    public AlgaeIntake()
    {
        if(Robot.isReal())
        {
            intakeIO = new AlgaeIntakeReal();
            return;
        }
        intakeIO = new AlgaeIntakeSim();
    }
    ...
    {
        AlgaeIntakeIO.setVoltage(RollerConstants.AlgaeConstants.algaeIntakePIDController.calculate(
        intakeData.velocityRadPerSec,goalVelocity) + RollerConstants.AlgaeConstants.algaeIntakePIDController.calculate(
        goalVelocity));
